...
function StaffDirectoryTab({ state, setState }) {
  ...
  const blankPerson = () => ({ name: "", idNumber: "", registrationNo: "", designation: "", email: "", contact: "", category: "staff", consultationTier: "", active: true, role: null });

  const openAdd = () => { setEditing(blankPerson()); setIsNew(true); setMsg(""); };
  const openEdit = (person) => { setEditing({ ...person, consultationTier: person.consultationTier || "" }); setIsNew(false); setMsg(""); };
  ...
  return (
    <div className="portal-bg">
      <div className="portal-container" style={{ maxWidth: "960px" }}>
        ...
        <div className="tab-bar">
          {["appointments", "active", "patients", "rooms", "memo"].map((t) => (
            <button key={t} className={`tab-btn${tab === t ? " active" : ""}`} onClick={() => setTab(t)}>
              {t === "appointments" ? "📋 Book Appointment" : t === "active" ? "📅 Active Appointments" : t === "patients" ? "👤 Patients" : t === "memo" ? "🧾 Memo" : "🏥 Rooms"}
            </button>
          ))}
        </div>
        {tab === "memo" && <MemoTab state={state} />}
        {tab === "rooms" && (
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
